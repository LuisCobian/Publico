...
if ($registro>0) 
{
	...
	$conexion=mysql_connect($host,$user)or die("no se conecto a la base");


	mysql_select_db($db)or die("no se pudo seleccionar la base de datos");
	
	$consultar="DELETE FROM  registros WHERE id='$registro' ";

	$ejecutas=mysql_query($consultar,$conexion)or die("no se pudo hacer la consulta");
	$bandera=true;
 ...
 ?>
		...
		<form action="baja-registros.php" method="post">
